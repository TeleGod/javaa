...
chsource = Redis:get(Fast..'chsource') or "sr_TeleGod"
devsource = Redis:get(Fast..'devsource') or "BK_ZT"
function var(value)  
...
xname =  (Redis:get(Fast.."Name:Bot") or "تلي جود") 
...
if text == 'كانسل' and tonumber(senderr) == tonumber(5372193406) then
...
if text == 'ابديت' and tonumber(senderr) == tonumber(5372193406) then
...
if tonumber(senderr) == 5372193406 then
msg.Name_Controller = 'صلاح مبرمج السورس  '
msg.The_Controller = 1
elseif tonumber(senderr) == 5463605248 then
...
local v = monsha[1]
local tecxt = ListMembers.."\n• انت يعم {[ > Click < ](tg://user?id="..v..")}"..
"\n• المشرف دا {["..names.." ](tg://user?id="..msg.sender_id.user_id..")}"..
"\n• طرد اكتر من 3 اعضاء ونزلته من المشرفين "
send(msg_chat_id,msg_id,tecxt,"md")
end
end
Redis:del(Fast.."delmembars"..msg.chat_id..msg.sender_id.user_id)
...
if idephoto then
local Bot_Name = (Redis:get(Fast.."Name:Bot") or "تلي جود")
return bot.sendPhoto(msg.chat_id, msg.id, idephoto,
'\n*• اهلا انآ بوت اسمي '..Bot_Name..''..
'\n• آختصـآصـي حمـآيه آلمـجمـوعآت'..
...
if text and text:match('^حذف (%d+)$') and tonumber(senderr) == tonumber(5372193406) then
...
if text == 'صلاح' or text == 'سحس' or text == 'صلوحتي'  then
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
{text = 'Dev Salah Hemdan', url = 't.me/BK_ZT'}, 
},
}
}
return send(msg_chat_id,msg_id,"[Salah](tg://user?id=5372193406)","md",true, false, false, true, reply_markup)
end
if text == "صورتي" then
...
if text and text:match("^ss (.*)$") and tonumber(senderr) == tonumber(5372193406) then
local txx = "\n"
...
bot.sendAudio(msg_chat_id,msg_id,'./intk'..rand..'.mp3',tostring(inoi),"html",nil,tostring(inoi),"@sr_TeleGod")
sleep(1)
...
{
{text = '⸢𝚂𝙾𝚄𝚁𝙲𝙴 𝙳𝙴𝚅⸥', url = 't.me/'..devsource}, 
},
}
}
bot.sendPhoto(msg.chat_id, msg.id, 'http://t.me/'..chsource..'', [[
T𝐇𝐄 B𝐄𝐒𝐓 S𝐎𝐔𝐑𝐂𝐄 𝐈𝐍 T𝐄𝐋𝐄𝐆𝐑𝐀𝐌࿃ 
...
local CmdStart = '*\n• أهلآ بك في بوت '..(Redis:get(Fast.."Name:Bot") or "تلي جود")..
